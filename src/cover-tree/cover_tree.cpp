...
void CoverTree::kNearestNeighbours(CoverTree::Node *current, double dist_current, const pointType &p, std::vector<std::pair<CoverTree::Node *, double>> &nnList) const
{
    ...
    {
        auto comp_x = [](std::pair<CoverTree::Node *, double> a, std::pair<CoverTree::Node *, double> b)
...
{
    // Do the worst initialization - use nullptr instead of allocating dummy nodes
    std::pair<CoverTree::Node *, double> dummy(nullptr, std::numeric_limits<double>::max());
    ...
    // Filter out nullptr entries before returning
    nnList.erase(std::remove_if(nnList.begin(), nnList.end(),
                                [](const std::pair<CoverTree::Node *, double> &p)
                                {
...
void CoverTree::deserialize(char *buff)
{
    ...
    char *post = buff + sizeof(pointType::Scalar) * D * N + sizeof(int) * N;
...
CoverTree::CoverTree(const int truncate /*=-1*/)
    : root(NULL), min_scale(1000), max_scale(0), truncate_level(truncate), id_valid(false), N(0), D(0)
{
}

// constructor: needs at least 1 point to make a valid cover-tree
CoverTree::CoverTree(const pointType &p, int truncate /*=-1*/)
    : min_scale(1000), max_scale(0), truncate_level(truncate), id_valid(false), N(1), D(p.rows())
{
